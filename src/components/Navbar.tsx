...
export function Navbar({ onOpenHistory, onOpenApi, historyCount }: NavbarProps) {
  const [isToolsOpen, setIsToolsOpen] = useState(false);

  const tools = [
    { name: 'YouTube Downloader', href: '/youtube-downloader' },
    { name: 'Spotify Downloader', href: '/spotify-downloader' },
    { name: 'TikTok Downloader', href: '/tiktok-downloader' },
    { name: 'Instagram Downloader', href: '/instagram-downloader' },
    { name: 'X / Twitter Downloader', href: '/twitter-downloader' },
      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[11px] text-neutral-400">Universal Media & Audio Downloader</span>
            </div>
          </Link>
        </div>
        ...
        <nav className="hidden md:flex items-center gap-6 text-xs font-semibold text-neutral-300">
          ...
          <div className="relative">
            ...
            {isToolsOpen && (
              <div className="absolute top-full left-0 mt-2 w-52 rounded-2xl border border-neutral-800 bg-neutral-900/95 p-2 shadow-2xl backdrop-blur-xl animate-in fade-in zoom-in-95 duration-150 z-50">
                {tools.map((t) => (
                  <Link
                    key={t.href}
                    href={t.href}
        ...
        <div className="flex items-center gap-2.5">
          <Link
            ...
            title="Live Provider Health Monitoring"
            className="hidden sm:flex items-center gap-1 rounded-xl border border-neutral-800 bg-neutral-900/60 px-2.5 py-2 text-[11px] font-medium text-neutral-400 hover:text-emerald-400 hover:border-emerald-500/30 transition-colors"
          >
