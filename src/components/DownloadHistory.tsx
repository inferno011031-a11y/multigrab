      ...
      <div className="relative flex h-full w-full max-w-md flex-col bg-neutral-950 border-l border-neutral-800 p-6 shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-neutral-800 pb-4">
          <div className="flex items-center gap-2.5">
            ...
            <span className="rounded-full bg-neutral-900 border border-neutral-800 px-2 py-0.5 text-xs text-neutral-300">
              {history.length}
            </span>
          </div>
        ...
        <div className="mt-4 flex-1 overflow-y-auto space-y-3 pr-1">
          {history.length === 0 ? (
            <div className="flex h-64 flex-col items-center justify-center text-center text-neutral-500">
              <Film className="h-10 w-10 stroke-[1.5] mb-2 opacity-40 text-white" />
            ...
            history.map((item) => (
              <div
                key={item.id}
                className="group relative flex flex-col rounded-xl border border-neutral-800 bg-neutral-900/40 p-3.5 transition-all hover:border-neutral-700 hover:bg-neutral-900"
              >
                ...
                <div className="mt-2 flex items-center justify-between text-[11px] text-neutral-400">
                  <span className="uppercase text-neutral-300 font-bold">{item.platform}</span>
                  <div className="flex items-center gap-2">
                ...
                <div className="mt-3 flex items-center gap-2 pt-2 border-t border-neutral-800">
                  <button
                    onClick={() => onSelectUrl(item.url)}
                    className="text-[11px] font-medium text-white hover:underline cursor-pointer"
                  >
                    Analyze Again
                  </button>
                  {item.downloadUrl && (
